...
from models import Contest, Problem, ProblemModel, Standings, StandingsData
...
def fit_problem_model(user_results, task_screen_name) -> ProblemModel:
    max_score = max(
    ...
    if len(time_model_sample_users) < 5:
        ...
        if slope > 0:
            logger.info("slope is positive. ignoring unreliable estimation.")
        else:
            model.slope = slope
    ...
    if len(difficulty_dataset) < 40:
        ...
        logger.info("all contestants got AC. skip estimating difficulty model.")
    elif not any(
        task_result[task_screen_name + ".ac"] for task_result in difficulty_dataset
    ):
        logger.info("no contestants got AC. skip estimating difficulty model.")
    else:
        d_raw_ratings = [
        ...
        logger.info(f"difficulty: {difficulty}, discrimination: {discrimination}")
        if discrimination < 0:
            logger.info("discrimination is negative. ignoring unreliable estimation.")
        elif difficulty > 6000:
            logger.info("extreme difficulty. rejecting this estimation.")
        else:
...
def fetch_dataset_for_contest(
    ...
    contest_type,
    existing_problem,
    skip_if_no_user_has_rating,
):
    try:
        response = s3.Object(
            "atcoder-problems-2025-data", f"standings/{contest_name}.json"
        ).get()
        results = Standings.model_validate_json(response["Body"].read().decode("utf-8"))
    ...
    user_results_by_problem: dict[str, list[dict]] = defaultdict(list)
...
def get_current_models() -> dict[str, ProblemModel]:
    try:
        ...
        logger.error(f"Failed to fetch existing models.\n{e}")
        return {}
...
def all_rated_contests() -> list[tuple[str, ContestType]]:
    # Gets all contest IDs and their contest type
    # The result is ordered by the start time.
    response = requests.get(
        "https://kenkoooo.com/atcoder/resources/contests.json"
    ).json()
    ta = TypeAdapter(list[Contest])
    contests = ta.validate_python(response)
...
def all_contest_problems():
    ...
    ta = TypeAdapter(list[Problem])
    problems = ta.validate_python(response)
...
def run(target_contest_ids: list[str] | None, overwrite: bool):
    recompute_history = target_contest_ids is None and overwrite
    if target_contest_ids is None:
        target = all_rated_contests()
    else:
        all_contests = all_rated_contests()
        target = [
            contest for contest in all_contests if contest[0] in target_contest_ids
        ]
    current_models = get_current_models()
    existing_problems: set[str] = current_models.keys() if not overwrite else set()
    contest_problems = all_contest_problems()

    logger.info(f"Fetching dataset from {len(target)} contests.")
    ...
    for contest, contest_type in target:
        ...
        if recompute_history and is_old_contest:
            logger.info(f"Updating user rating with the result of {contest}")
            rating_system.update(standings, contest_type)
    logger.info(f"Estimating time models of {len(dataset_by_problem)} problems.")
