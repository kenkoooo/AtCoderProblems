...
class ContestType(enum.Enum):
    OLD_UNRATED_ABC = (
        ...
        800,
        False,
    )  # 4 problems ABC before official rating system
    OLD_UNRATED_ARC = (
        9999,
        9999,
        1200,
        False,
    )  # ARC *AND* sponsored contests before official rating system
...
def adjust_rating(raw_rating: float, n: int) -> int:
    ...
    if discounted_rating >= 400:
        ...
        return int(max(1.0, 400 / math.exp((400 - discounted_rating) / 400)))
...
class RatingSystem(object):
    ...
    def update(self, standings: List[str], contest_type: ContestType):
        ...
        qualified_contestants = [
            ...
            if (self.calc_rating(contestant) or 0) <= contest_type.qualified_bound
        ]
        a_perfs = [
            self.calc_a_perf(contestant, contest_type)
            for contestant in qualified_contestants
        ]
        ...
        for i, contestant in enumerate(qualified_contestants):
            ...
            while round(lb) < round(ub):
                ...
                if m not in predicted_rank_cache:
                    ...
                    for a_perf in a_perfs:
                        predicted_rank += 1.0 / (1.0 + (6.0 ** ((m - a_perf) / 400)))
            ...
            if is_first_contest:
                perf = (
                    perf - contest_type.default_perf
                ) * 1.5 + contest_type.default_perf
    ...
    def calc_rating(self, contestant: str) -> Optional[float]:
        if not self.past_rounded_performances[contestant]:
            return None
        exp_perf_sum, weight_sum = 0.0, 0.0
        for i, past_perf in enumerate(
            reversed(self.past_rounded_performances[contestant])
        ):
            weight = 0.9**i
            exp_perf = 2.0 ** (past_perf / 800)
            weight_sum += weight
