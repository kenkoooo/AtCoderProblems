...
export class Category extends React.Component<CategoryProps, {}>{
    ...
    private filterProblems(problemMap: Map<string, Array<Problem>>, regexp: RegExp): Array<[Contest, Array<Problem>]> {
    ...
    render() {
        ...
        let others = this.filterProblems(problemMap, /^(?!a[rgb]c\d{3}).*$/).sort((a, b) => {
            return b[0].start_epoch_second - a[0].start_epoch_second;
        });

        return (
            <Grid>
                <CategoryOneBlock categoryTitle="AtCoder Grand Contest" data={agc} header={"ABCDEF".split("")} />
                <CategoryOneBlock categoryTitle="AtCoder Beginner Contest" data={abc} header={"ABCD".split("")} />
                <CategoryOneBlock categoryTitle="AtCoder Regular Contest" data={arc} header={"ABCD".split("")} />
