...
class UserPage extends React.Component<Props, State> {
	...
	updateState(user_id: string) {
		Api.fetchSubmissions(user_id).then((submissions) => {
			...
			this.setState({ submissions, longest_streak, current_streak, last_ac, daily_data });
		});
	...
	render() {
		...
		const { submissions, user_info, longest_streak, current_streak, last_ac } = this.state;
		...
		const achievements = [
			...
			{ key: 'Shortest Code', value: shortest_rank.count, rank: shortest_rank.rank },
			{ key: 'Fastest Code', value: fastest_rank.count, rank: fastest_rank.rank },
...
const get_streak = (first_ac_map: Map<string, number>) => {
	...
	return { longest_streak, current_streak, last_ac: formatDate(last_ac_second) };
};
